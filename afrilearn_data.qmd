...
`afrilearndata` library has useful data for learning _**Spatial Analysis**_ in `R` within the African context. Having vector (with attribute data) and raster (with pixel values), the possibilities of analytic approaches are unlimited. I will barely **glimpse** tip of the iceberg.

You can find the rmd file in my [GitHub](https://github.com/Wycology/afrilearn_data/blob/main/afrilearn_data.qmd) account and more about the package [here](https://github.com/afrimapr/afrilearndata).

## Simple mapping of thecountries

```{r countries}
mf_init(africountries, expandBB = rep(0, 4), theme = 'iceberg')
mf_shadow(africountries,
          col = 'magenta',
          cex = 2,
          add = TRUE) # just for aesthetic
mf_layout(
  title = 'African countries map with shadow',
  credits = paste0(
...
## Mapping of population field as proportional circles

```{r pop_prop}
mf_init(africountries, expandBB = rep(0, 4), theme = 'dark')
mf_layout(
  title = 'African countries population estimate',
  credits = paste0(
    'Sources: IGN, 2018\nAuthor: Wyclife Agumba Oluoch\n',
    'Package: mapsf ',
    packageVersion('mapsf'),
...
mf_map(
  ...
  col = 'purple',
  leg_pos = 'topright',
  leg_title = 'Total population estimate'
)
```
...
mf_layout(
  title = 'African countries population estimate choropleth',
  credits = paste0(
    'Sources: Andy South, 2021\nAuthor: Wyclife Agumba Oluoch\n',
    'Package: mapsf ',
    packageVersion('mapsf'),
...
mf_map(
  ...
  lwd = 0.5,
  leg_pos = "left",
  leg_title = "Numeric estimated gdp",
  add = TRUE
)
...
```{r map_extracted}
mf_init(afripop2020_extract,
        expandBB = rep(0, 4),
        theme = 'candy')
mf_layout(
  title = 'African countries population density 2020',
  credits = paste0(
    'Sources: Andy South, 2021\nAuthor: Wyclife Agumba Oluoch\n',
    'Package: mapsf ',
    packageVersion('mapsf'),
...
Here I filter and map only countries whose pop_2020 values are above 100.

```{r pop_den_above_100}
afri_pop_2020_100 <- afripop2020_extract %>% filter(pop_2020 > 100)
mf_init(afri_pop_2020_100,
        expandBB = rep(0, 4),
        theme = 'dark')
mf_layout(
  title = 'African countries with > 100 people per km2 in 2020',
  credits = paste0(
    'Sources: Andy South, 2021\nAuthor: Wyclife Agumba Oluoch\n',
    'Package: mapsf ',
    packageVersion('mapsf'),
...
```{r pop_den_below_20}
afri_pop_2020_below_20 <-
  afripop2020_extract %>% filter(pop_2020 < 20)
mf_init(afri_pop_2020_below_20,
        expandBB = rep(0, 4),
        theme = 'dark')
mf_layout(
  title = 'African countries with below 20 people per km2 in 2020',
  credits = paste0(
    'Sources: Andy South, 2021\nAuthor: Wyclife Agumba Oluoch\n',
    'Package: mapsf ',
    packageVersion('mapsf'),
...
Here is a map of the country with max popln. density by 2020.

```{r max_pop_202}
afri_pop_2020_max <-
  afripop2020_extract %>% filter(pop_2020 == max(pop_2020, na.rm = TRUE))
mf_init(afri_pop_2020_max,
        expandBB = rep(0, 4),
        theme = 'dark')
mf_layout(
  title = 'African country with maximum people per km2 in 2020',
  credits = paste0(
    'Sources: Andy South, 2021\nAuthor: Wyclife Agumba Oluoch\n',
    'Package: mapsf ',
    packageVersion('mapsf'),
...
```{r min_pop_2020}
afri_pop_2020_min <-
  afripop2020_extract %>% filter(pop_2020 == min(pop_2020, na.rm = TRUE))
mf_init(afri_pop_2020_min,
        expandBB = rep(0, 4),
        theme = 'dark')
mf_layout(
  title = 'African countries with minimum people per km2 in 2020',
  credits = paste0(
    'Sources: Andy South, 2021\nAuthor: Wyclife Agumba Oluoch\n',
    'Package: mapsf ',
    packageVersion('mapsf'),
...
kenya_mask <-
  raster::mask(raster::crop(raster::raster(land_cover), kenya_sp), kenya_sp)
...
```{python}
import geopandas as gpd # Working with geopandas 
