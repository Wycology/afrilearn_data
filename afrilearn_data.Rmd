...
`afrilearndata` package has go data that can be used to learn more about Spatial Analysis in `R` especially within the African context. Having vector (with attribute data) and raster (with pixel values), the possibilities of analytic approaches are unlimited. Here, I will barely glimpse tip of the iceberg.
